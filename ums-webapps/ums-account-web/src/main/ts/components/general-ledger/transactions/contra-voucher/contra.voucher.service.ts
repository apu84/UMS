  ...
  export class ContraVoucherService {
    ...
    public getVouchersByVoucherNoAndDate(voucherNo: string, date: string): ng.IPromise<IContraVoucher[]> {
      ...
      public generateVoucherReport(voucherNo: string, voucherDate: string): ng.IPromise<string> {
          ...
          this.httpClient.get(this.url + "/contraVoucherReport/voucherNo/"+voucherNo+"/voucherDate/"+voucherDate, undefined, (data: any, etag: string) => {
                  UmsUtil.writeFileContent(data, contentType, 'Contra Voucher ('+voucherDate +').pdf');
              },
              (response: any) => {
