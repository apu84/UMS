...
public class PersistentExamRoutineDao extends ExamRoutineDaoDecorator {
  ...
  public List<ExamRoutineDto> getExamRoutineForApplicationCCI(int semesterId, int examType) {
    ...
    return mJdbcTemplate.query(query, new Object[] {examType, semesterId, semesterId},
        new ExamRoutineForCCIRowMapper());
  }
  ...
  public ExamRoutineDto getExamRoutineForCivilExamBySemester(Integer pSemesterId) {
    ...
    return mJdbcTemplate.queryForObject(query, new Object[] {pSemesterId},
  ...
  public List<ExamRoutineDto> getExamRoutineBySemesterAndExamTypeOrderByExamDateAndProgramIdAndCourseId(
    ...
    String query =
        "" + "SELECT " + "  SEMESTER, " + "  to_char(EXAM_DATE,'DD/MM/YYYY') exam_date, "
            + "  EXAM_TIME, " + "  PROGRAM_ID, " + "  COURSE_ID " + "FROM EXAM_ROUTINE " + "WHERE "
            + "  SEMESTER=? " + "  AND EXAM_TYPE=? " + "ORDER BY EXAM_DATE, " + "  PROGRAM_ID, "
            + "  COURSE_ID";
    return mJdbcTemplate.query(query, new Object[] {pSemesterId, pExamType},
        new ExamRoutineRowMapperOfDbTable());
  }
  ...
  public int delete(final MutableExamRoutine pExamRoutine) {
    if(pExamRoutine.getInsertType().equalsIgnoreCase("byProgram")) {
      ...
      return mJdbcTemplate.update(query, pExamRoutine.getSemesterId(),
          pExamRoutine.getExamTypeId(), examDate, examTime, programId);
    }
      ...
      String examTime = pExamRoutine.getRoutine().get(0).getExamTime();
      return mJdbcTemplate.update(query, pExamRoutine.getSemesterId(),
          pExamRoutine.getExamTypeId(), examDate, examTime);
    }
    else if(pExamRoutine.getInsertType().equalsIgnoreCase("all")) {
      String query = DELETE;
      return mJdbcTemplate
          .update(query, pExamRoutine.getSemesterId(), pExamRoutine.getExamTypeId());
    }
    return 0;
  }
