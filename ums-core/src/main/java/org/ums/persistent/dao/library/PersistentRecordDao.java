...
public class PersistentRecordDao extends RecordDaoDecorator {
  static String SELECT_ALL =
      "Select MFN, LANGUAGE, TITLE, SUB_TITLE, GMD, SERIES_TITLE, VOLUME_NO, VOLUME_TITLE, SERIAL_ISSUE_NO,  "
          ...
          + "   DATE_YEAR_OF_PUBLICATION, COPY_RIGHT_DATE, COPY_RIGHT_DATE, MATERIAL_TYPE, STATUS, BINDING_TYPE,  "
  ...
  static String UPDATE_ONE =
      "UPDATE RECORDS SET LANGUAGE = ?, TITLE=?,  SUB_TITLE=?, GMD=?, SERIES_TITLE=?, VOLUME_NO=?, VOLUME_TITLE=?, SERIAL_ISSUE_NO=?, SERIAL_NUMBER=?, "
          ...
          + "  STATUS=?, BINDING_TYPE=?, KEYWORDS=?, CONTRIBUTORS=?, SUBJECTS=?, PHYSICAL_DESC = ?, NOTES=?, TOTAL_ITEMS = ?, TOTAL_AVAILABLE = ?, TOTAL_CHECKED_OUT = ?, TOTAL_ON_HOLD = ?, LAST_UPDATED_ON=sysdate,  "
  ...
  static String INSERT_ONE =
      "INSERT INTO RECORDS(MFN, LANGUAGE, TITLE, SUB_TITLE, GMD, SERIES_TITLE,  VOLUME_NO, VOLUME_TITLE, SERIAL_ISSUE_NO,SERIAL_NUMBER, "
          ...
          + "    STATUS, BINDING_TYPE, KEYWORDS, DOCUMENTALIST,CONTRIBUTORS, SUBJECTS,PHYSICAL_DESC, NOTES, TOTAL_ITEMS, TOTAL_AVAILABLE, TOTAL_CHECKED_OUT, TOTAL_ON_HOLD,  ENTRY_DATE, LAST_UPDATED_ON, LAST_UPDATED_BY, LAST_MODIFIED)  "
          + "  VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,"
          + " ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
          + " ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
          + " ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, 0, 0,"
  ...
  public int update(final MutableRecord pRecord) {
    ...
    return mJdbcTemplate.update(query, pRecord.getLanguage() == null ? null : pRecord.getLanguage().getId(), pRecord
        ...
        pRecord.getBookBindingType() == null ? null : pRecord.getBookBindingType().getId(), pRecord.getKeyWords(),
        pRecord.getContributorJsonString(), pRecord.getSubjectJsonString(), pRecord.getPhysicalDescriptionString(),
  ...
  public Long create(final MutableRecord pRecord) {
    ...
    mJdbcTemplate.update(INSERT_ONE, pRecord.getMfn(), pRecord.getLanguage().getId(), pRecord.getTitle(), pRecord
        ...
        pRecord.getRecordStatus().getId(), pRecord.getBookBindingType().getId(), pRecord.getKeyWords(), pRecord
