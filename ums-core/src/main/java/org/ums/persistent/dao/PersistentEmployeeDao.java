...
public class PersistentEmployeeDao  extends EmployeeDaoDecorator{
  ...
  static String SELECT_ALL = "SELECT EMPLOYEE_ID,EMPLOYEE_NAME,DESIGNATION,EMPLOYMENT_T" +
      "YPE,DEPT_OFFICE,FATHER_NAME,MOTHER_NAME,BIRTH_DATE,GENDER,BLOOD_GROUP," +
  ...
  static String INSERT_ONE = "INSERT INTO EMPLOYEES (EMPLOYEE_NAME,DESIGNATION,EMPLOYMENT_TYPE,DEPT_OFFICE," +
      ...
      "values (?,?,?,?,?,?,to_date(?,'DD-MON-RR'),?,?,?,?,?,?,?,to_date(?,'DD-MON-RR'),?,?,"+getLastModifiedSql() +")";

  static String UPDATE_ONE = "UPDATE EMPLOYEES SET EMPLOYEE_NAME=?,DESIGNATION=?,EMPLOYMENT_TYPE=?,DEPT_OFFICE=?," +
      "FATHER_NAME=?,MOTHER_NAME=?,BIRTH_DATE=?,GENDER=?,BLOOD_GROUP=?,PRESENT_ADDRESS=?,PERMANENT_ADDRESS=?," +
      "MOBILE_NUMBER=?,PHONE_NUMBER=?,EMAIL_ADDRESS=?,JOINING_DATE=?,JOB_PERMANENT_DATE=?,STATUS=? ,LAST_MODIFIED="+getLastModifiedSql()+" ";
  ...
  public int update(MutableEmployee pMutable) throws Exception {
    ...
    return mJdbcTemplate.update(query,
        pMutable.getEmployeeName(),
        pMutable.getDesignation(),
        pMutable.getEmploymentType(),
        pMutable.getDepartment().getId(),
        pMutable.getFatherName(),
        pMutable.getMotherName(),
        pMutable.getBirthDate(),
        pMutable.getGender(),
        pMutable.getBloodGroup(),
        pMutable.getPresentAddress(),
        pMutable.getPermanentAddress(),
        pMutable.getMobileNumber(),
        pMutable.getPhoneNumber(),
        pMutable.getEmailAddress(),
        pMutable.getJoiningDate(),
        pMutable.getJobPermanentDate(),
        pMutable.getStatus(),
        pMutable.getLastModified(),
        pMutable.getId()
        );
  }
  ...
  public Employee getByEmployeeId(String pEmployeeId) {
    ...
    return mJdbcTemplate.queryForObject(query,new Object[]{pEmployeeId},new EmployeeRowmapper());
  }
