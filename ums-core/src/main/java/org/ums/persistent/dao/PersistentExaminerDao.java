...
public class PersistentExaminerDao extends AbstractAssignedTeacherDao<Examiner, MutableExaminer, Integer> {
  static String SELECT_ALL = "SELECT SEMESTER_ID, PREPARER, SCRUTINIZER, COURSE_ID, LAST_MODIFIED, ID FROM PREPARER_SCRUTINIZER ";
  static String UPDATE_ALL = "UPDATE PREPARER_SCRUTINIZER SET SEMESTER_ID = ?, PREPARER = ?, SCRUTINIZER= ?, COURSE_ID = ?, LAST_MODIFIED = " + getLastModifiedSql() + " ";
  static String DELETE_ALL = "DELETE FROM PREPARER_SCRUTINIZER ";
  static String INSERT_ONE = "INSERT INTO PREPARER_SCRUTINIZER(SEMESTER_ID, PREPARER, SCRUTINIZER, COURSE_ID, LAST_MODIFIED) VALUES" +
      "(?, ?, ?, ?," + getLastModifiedSql() + ")";
  ...
  public int create(MutableExaminer pMutable) throws Exception {
    return mJdbcTemplate.update(INSERT_ONE,
        ...
        pMutable.getPreparer().getId(),
        pMutable.getScrutinizer().getId(),
