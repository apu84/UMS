...
public class PersistentSeatPlanDao extends SeatPlanDaoDecorator {
  ...
  public List<SeatPlan> getBySemesterGroupTypeRoomRowAndCol(int pSemesterId, int pGroupNo, int pType, int pRoomId,
    ...
    return mJdbcTemplate.query(query, new Object[] {pSemesterId, pGroupNo, pType, pRoomId, pRow, pCol},
  ...
  public List<SeatPlan> getByRoomSemesterGroupExamType(int pRoomId, int pSemesterId, int pGroupNo, int pExamType) {
    ...
    return mJdbcTemplate
        .query(query, new Object[] {pRoomId, pSemesterId, pGroupNo, pExamType}, new SeatPlanRowMapper());
  }
  ...
  public List<SeatPlan> getBySemesterAndGroupAndExamTypeAndExamDate(int pSemesterId, int pGropuNo, int pExamType,
      String pExamDate) {
    String query =
        SELECT_ALL_CCI + " WHERE SEMESTER_ID=? AND GROUP_NO=? AND EXAM_TYPE=? AND exam_date=to_date(?,'MM-DD-YYYY')";
  ...
  public List<SeatPlan> getSeatPlanOrderByExamDateAndCourseAndYearAndSemesterAndStudentId(Integer pSemesterId,
    ...
    String query =
        "" + "SELECT  " + "  SEAT_PLAN.ID,  " + "  SEAT_PLAN.ROOM_ID,  " + "  SEAT_PLAN.ROW_NO,  "
            ...
            + "  SEAT_PLAN.SEMESTER_ID,  " + "  SEAT_PLAN.GROUP_NO,  " + "  SEAT_PLAN.LAST_MODIFIED  " + "FROM  "
            + "  SEAT_PLAN,STUDENTS,  " + "  (  " + "SELECT  " + "  EXAM_ROUTINE.EXAM_DATE,  "
            + "  MST_COURSE.COURSE_ID,  " + "  MST_COURSE.YEAR,  " + "  MST_COURSE.SEMESTER  " + "  "
            + "FROM MST_COURSE,EXAM_ROUTINE  " + "WHERE  " + "  EXAM_ROUTINE.SEMESTER=?  " + "  AND EXAM_TYPE=?  "
            + "  AND EXAM_ROUTINE.COURSE_ID=MST_COURSE.COURSE_ID  " + "  " + "ORDER BY  "
            + "  EXAM_ROUTINE.EXAM_DATE,  " + "  EXAM_ROUTINE.PROGRAM_ID,  " + "  EXAM_ROUTINE.COURSE_ID,  "
            + "  MST_COURSE.YEAR,  " + "  MST_COURSE.SEMESTER  " + "    ) examRoutine  " + "WHERE  "
            + "  SEAT_PLAN.EXAM_TYPE=? AND  " + "    SEAT_PLAN.SEMESTER_ID=? and  "
            + "  SEAT_PLAN.STUDENT_ID=STUDENTS.STUDENT_ID  " + "  AND examRoutine.YEAR=STUDENTS.CURR_YEAR  "
            + "  AND examRoutine.SEMESTER=STUDENTS.CURR_SEMESTER  " + "ORDER BY  " + "  examRoutine.EXAM_DATE,  "
            + "  examRoutine.COURSE_ID,  " + "  examRoutine.YEAR,  " + "  examRoutine.SEMESTER,  "
            + "  STUDENTS.STUDENT_ID";
  ...
  public int deleteBySemesterGroupExamTypeAndExamDate(int pSemesterId, int pGroupNo, int pExamType, String pExamDate) {
    String query =
        DELETE_ALL + " WHERE SEMESTER_ID=? AND GROUP_NO=? AND EXAM_TYPE=? AND exam_date=to_date(?,'MM-DD-YYYY')";
  ...
  public List<SeatPlan> getForStudentAndCCIExam(String pStudentId, Integer pSemesterid, String pExamDate) {
    String query =
        "SELECT s.* FROM seat_plan s, EXAM_ROUTINE e "
            + "WHERE s.student_id = ? AND s.exam_type = 2 AND s.semester_id = ? "
            + "AND e.EXAM_DATE = TO_DATE (?, 'dd-mm-yyyy') AND e.SEMESTER = s.SEMESTER_ID "
            + "AND e.COURSE_ID = s.COURSE_ID ORDER BY row_no, col_no";
  ...
  public List<SeatPlan> getSittingArrangement(int pSemesterId, ExamType pExamType) {
    ...
    return mJdbcTemplate.query(query, new Object[] {pSemesterId, pExamType.getId()}, new SeatPlanRowMapper());
  ...
  public Long create(MutableSeatPlan pMutable) {
    ...
    mJdbcTemplate.update(INSERT_ALL, mIdGenerator.getNumericId(), pMutable.getClassRoom().getId(), pMutable
        .getSemester().getId(), pMutable.getGroupNo(), pMutable.getStudent().getId(), pMutable.getRowNo(), pMutable
        .getColumnNo(), pMutable.getExamType());
    return id;
  }
  ...
  private List<Object[]> getInsertParamList(List<MutableSeatPlan> pSeatPlans) {
    ...
    for(SeatPlan seatPlan : pSeatPlans) {
      params.add(new Object[] {mIdGenerator.getNumericId(), seatPlan.getClassRoom().getId(),
          seatPlan.getSemester().getId(), seatPlan.getGroupNo(), seatPlan.getStudent().getId(), seatPlan.getRowNo(),
          seatPlan.getColumnNo(), seatPlan.getExamType()});
    }
    return params;
  }
