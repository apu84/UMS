...
public class PersistentExamGradeDao  extends ExamGradeDaoDecorator {
  ...
  String SELECT_PART_INFO="Select MARKS_SUBMISSION_STATUS.*, LAST_SUBMISSION_DATE,COURSE_TYPE,COURSE_TITLE,COURSE_NO,CRHR,LONG_NAME,SEMESTER_NAME " +
  ...
  String SELECT_GRADE_SUBMISSION_TABLE_TEACHER="Select tmp5.*,Status,Exam_Type, last_submission_date From ( " +
  ...
  String SELECT_GRADE_SUBMISSION_TABLE_HEAD="Select last_submission_date, Ms_Status.Semester_Id,Exam_Type,Mst_Course.Course_Id,Course_No,Course_Title ,CrHr,Course_Type,Course_Category,Offer_By,Ms_Status.Year,Ms_Status.Semester,COURSE_SYLLABUS_MAP.Syllabus_Id, " +
  ...
  String SELECT_GRADE_SUBMISSION_TABLE_CoE="Select last_submission_date, Ms_Status.Semester_Id,Exam_Type,Mst_Course.Course_Id,Course_No,Course_Title ,CrHr,Course_Type,Course_Category,Offer_By,Ms_Status.Year,Ms_Status.Semester,COURSE_SYLLABUS_MAP.Syllabus_Id, " +
      "getCourseTeacher(Ms_Status.semester_id,Mst_Course.course_id) Course_Teachers, " +
      "MST_PROGRAM.PROGRAM_SHORT_NAME,getPreparerScrutinizer(Ms_Status.Semester_Id,Mst_Course.Course_Id,'P') PREPARER_NAME, " +
      "getPreparerScrutinizer(Ms_Status.Semester_Id,Mst_Course.Course_Id,'S') SCRUTINIZER_NAME, STATUS  " +
      "From MARKS_SUBMISSION_STATUS Ms_Status,Mst_Course,COURSE_SYLLABUS_MAP,MST_SYLLABUS,MST_PROGRAM " +
  ...
  String UPDATE_STATUS_SAVE_RECHECK="Update % Set RECHECK_STATUS=?  Where SEMESTER_ID=? And COURSE_ID=? And EXAM_TYPE=? And STUDENT_ID=? and " +
      " Status in (select regexp_substr(?,'[^,]+', 1, level) from dual connect by regexp_substr(?, '[^,]+', 1, level) is not null)";
  String UPDATE_STATUS_SAVE_APPROVE="Update % Set RECHECK_STATUS=?,STATUS=?  Where SEMESTER_ID=? And COURSE_ID=? And EXAM_TYPE=? And STUDENT_ID=? and " +
      " Status in  (select regexp_substr(?,'[^,]+', 1, level) from dual connect by regexp_substr(?, '[^,]+', 1, level) is not null)";

  String UPDATE_STATUS_RECHECK_RECHECK="Update % Set RECHECK_STATUS=?,STATUS=?   Where SEMESTER_ID=? And COURSE_ID=? And EXAM_TYPE=? And STUDENT_ID=? and " +
      " Status in (select regexp_substr(?,'[^,]+', 1, level) from dual connect by regexp_substr(?, '[^,]+', 1, level) is not null)";
  String UPDATE_STATUS_RECHECK_APPROVE="Update % Set RECHECK_STATUS=?,STATUS=?   Where SEMESTER_ID=? And COURSE_ID=? And EXAM_TYPE=? And STUDENT_ID=? and  " +
      " Status in (select regexp_substr(?,'[^,]+', 1, level) from dual connect by regexp_substr(?, '[^,]+', 1, level) is not null)";

  String UPDATE_STATUS_APPROVE="Update % Set RECHECK_STATUS=?, STATUS=?  Where SEMESTER_ID=? And COURSE_ID=? And EXAM_TYPE=? And STUDENT_ID=? and  " +
      " Status in (select regexp_substr(?,'[^,]+', 1, level) from dual connect by regexp_substr(?, '[^,]+', 1, level) is not null)";
  ...
  @Override
  public boolean saveGradeSheet(MarksSubmissionStatus actualStatus, List<StudentGradeDto> pGradeList) throws Exception {
    batchUpdateGrade(actualStatus.getSemesterId(), actualStatus.getCourseId(), actualStatus.getExamType(),
        actualStatus.getCourse().getCourseType(), pGradeList);
    return true;
  }
  ...
  public boolean updateGradeStatus_Save(MarksSubmissionStatus actualStatus,
                                        List<StudentGradeDto> recheckList,
                                        List<StudentGradeDto> approveList) throws Exception {
  ...
  @Override
  public boolean updateGradeStatus_Recheck(MarksSubmissionStatus actualStatus, List<StudentGradeDto> recheckList, List<StudentGradeDto> approveList) throws Exception {
  ...
  public int rejectRecheckRequest(MarksSubmissionStatus pMarksSubmissionStatus) throws Exception {
    ...
    return mJdbcTemplate.update(query,
        ...
        pMarksSubmissionStatus.getSemesterId(),
        pMarksSubmissionStatus.getCourseId(),
  ...
  public int approveRecheckRequest(MarksSubmissionStatus pMarksSubmissionStatus) throws Exception {
    String query = "UPDATE %s SET STATUS = ? WHERE SEMESTER_ID = ? AND COURSE_ID = ? AND EXAM_TYPE = ? AND STATUS = ? "
        + "AND RECHECK_STATUS= ?";
    query = String.format(query, getTableName(pMarksSubmissionStatus.getCourse().getCourseType()));
    return mJdbcTemplate.update(query,
        ...
        pMarksSubmissionStatus.getCourseId(),
        pMarksSubmissionStatus.getExamType().getId(),
        StudentMarksSubmissionStatus.ACCEPTED.getId(),
  ...
  private int batchUpdateGradeStatus_Recheck(MarksSubmissionStatus actualStatus,
    ...
    int[] recheckTotal;
    int[] approveTotal;
    String query = getQuery(UPDATE_STATUS_RECHECK_RECHECK, actualStatus.getCourse().getCourseType());
  ...
  @Override
  public boolean updateGradeStatus_Approve(MarksSubmissionStatus actualStatus, List<StudentGradeDto> recheckList, List<StudentGradeDto> approveList) throws Exception {
  ...
  private void batchInsertGradeLog(String userId,
                                   String pRole,
  ...
  @Override
  public int insertMarksSubmissionStatusLog(String userId,
                                            String userRole,
                                            MarksSubmissionStatus actualStatus,
    ...
    return mJdbcTemplate.update(INSERT_MARKS_SUBMISSION_STATUS_LOG,
        userId,
        userRole,
        actualStatus.getSemesterId(),
        actualStatus.getCourseId(),
