...
public class PersistentSubGroupCCIDao  extends SubGroupCCIDaoDecorator{
  ...
  String INSERT_ONE= "INSERT INTO SP_SUB_GROUP_CCI (SEMESTER_ID,SUB_GROUP_NO,TOTAL_STUDENT,COURSE_ID,EXAM_DATE,LAST_MODIFIED)" +
      " VALUES (?,?,?,?,?,"+getLastModifiedSql()+")";
  String DELETE_ONE = "DELETE FROM SP_SUB_GROUP_CCI ";
  String UPDATE_ONE = " UPDATE SP_SUB_GROUP_CCI SET SEMESTER_ID=?, SUB_GROUP_NO=?,TOTAL_STUDENT=?,COURSE_ID=?,EXAM_DATE=?,LAST_MODIFIED="+getLastModifiedSql();
  String SELECT_ALL_SUB_GROUP = "select s.id,s.semester_id,s.sub_group_no,s.total_student,s.course_id,s.exam_date,c.course_no,c.year,c.semester,s.last_modified  " +
  ...
  public Integer checkForHalfFinishedSubGroup(Integer pSemesterId, String pExamDate) {
    String query = "select count(*) from sp_sub_group_cci where exam_date=to_date(?,'MM-DD-YYYY') and semester_id=? and sub_group_no=0";
    return mJdbcTemplate.queryForObject(query,Integer.class,pExamDate,pSemesterId);
  }

  @Override
  public Integer checkSubGroupNumber(Integer pSemesterId, String pExamDate) {
    String query = "select count(sub_group_no) from ( " +
  ...
  public int create(List<MutableSubGroupCCI> pMutableList) throws Exception {
    String query= "INSERT INTO SP_SUB_GROUP_CCI (SEMESTER_ID,SUB_GROUP_NO,TOTAL_STUDENT,COURSE_ID,EXAM_DATE,LAST_MODIFIED)" +
  ...
  private List<Object[]> getInsertParamList(List<MutableSubGroupCCI> pMutableSubGroupCCIs)throws Exception{
    List<Object[]> params = new ArrayList<>();
    for(SubGroupCCI subGroup:pMutableSubGroupCCIs){
      params.add(new Object[]{
          subGroup.getSemesterId(),
          subGroup.getSubGroupNo(),
          subGroup.getTotalStudent(),
          subGroup.getCourseId(),
          subGroup.getExamDate()
      });
    }
    return params;
  }
