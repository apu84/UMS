...
public class ParameterBuilder implements Builder<Parameter,MutableParameter> {
  @Override
  public void build(JsonObjectBuilder pBuilder, Parameter pReadOnly, UriInfo pUriInfo, LocalCache pLocalCache) throws Exception {
    ...
    pBuilder.add("self", pUriInfo.getBaseUriBuilder().path("academic").path("academicCalenderParameter").path(pReadOnly.getId().toString()).build().toString());
  }

  @Override
  public void build(MutableParameter pMutable, JsonObject pJsonObject, LocalCache pLocalCache) throws Exception {
