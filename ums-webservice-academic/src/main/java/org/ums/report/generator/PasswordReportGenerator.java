...
public class PasswordReportGenerator extends AbstractReportGenerator {
  ...
  /*
   * @Override public MasterReport getReportDefinition() throws Exception { // Using the
   * classloader, get the URL to the reportDefinition file final ClassLoader classloader =
   * this.getClass().getClassLoader(); final URL reportDefinitionURL =
   * classloader.getResource("report/password/generatedPassword.prpt");
   * 
   * // Parse the report file final ResourceManager resourceManager = new ResourceManager(); final
   * Resource directly = resourceManager.createDirectly(reportDefinitionURL, MasterReport.class);
   * return (MasterReport) directly.getResource(); }
   * 
   * public DataFactory getDataFactory(String reportQuery) { // TODO : Refactor this connection to
   * use the common connection used for the application final DriverConnectionProvider
   * sampleDriverConnectionProvider = new DriverConnectionProvider();
   ...
   * SQLReportDataFactory(sampleDriverConnectionProvider); dataFactory.setQuery(QUERY_NAME,
   * reportQuery);
   * 
   * return dataFactory; }
   */
