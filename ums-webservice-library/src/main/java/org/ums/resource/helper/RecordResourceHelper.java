    ...
    private String queryBuilder(final String pFilter) {
        ...
        if (filterDto.getSearchType().equalsIgnoreCase("basic")) {
            ...
            for (int i = 0; i < sizeOfMap; i++) {
                sb.append(filterDto.getAdvancedSearchFilter().get(i).get("key")).append(":").append(filterDto.getAdvancedSearchFilter().get(i).get("value").isEmpty() ? "*" :
                        filterDto.getAdvancedSearchFilter().get(i).get("value")).append(" ").append(sizeOfMap - 1 == i ? "" : "AND ");
            }
