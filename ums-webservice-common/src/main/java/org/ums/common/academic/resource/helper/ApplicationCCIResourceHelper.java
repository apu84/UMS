...
import org.ums.common.validator.ApplicationCCIValidator;
...
public class ApplicationCCIResourceHelper extends ResourceHelper<ApplicationCCI,MutableApplicationCCI,Integer>{
  ...
  public Response post(JsonObject pJsonObject, UriInfo pUriInfo) throws Exception {
    /*Validator validate = new ApplicationCCIValidator();
    validate.validate(pJsonObject);*/
  ...
  public JsonObject saveAndReturn(JsonObject pJsonObject, UriInfo pUriInfo) throws Exception {
    ...
    List<UGRegistrationResult> results = mResultManager.getCarryClearanceImprovementCoursesByStudent(student.getSemesterId(),studentId);
  ...
  public Response deleteByStudentId(UriInfo pUriInfo)throws Exception{
    ...
    URI contextURI = pUriInfo.getBaseUriBuilder().path(ApplicationCCIResource.class).path(ApplicationCCIResource.class,"get").build();
    Response.ResponseBuilder builder = Response.created(contextURI);
    builder.status(Response.Status.CREATED);
    return builder.build();
  }

  public JsonObject getApplicationCCIInfoForStudent(final Request pRequest, final UriInfo pUriInfo) throws Exception{
    String studentId = SecurityUtils.getSubject().getPrincipal().toString();
    Student student = mStudentManager.get(studentId);
    ...
    if(applictionAll.size()>0){
      List<ApplicationCCI> applications = getContentManager().getByStudentIdAndSemester(studentId,student.getSemesterId());
