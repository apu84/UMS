...
public class PasswordReport extends Resource {
  ...
  public StreamingOutput get(final @Context Request pRequest, final @PathParam("object-id") String pObjectId) throws Exception {
    return new StreamingOutput() {
      public void write(OutputStream output) throws IOException, WebApplicationException {
        try {
          String query="Select User_Id,Temp_Password,Full_Name,'Student' Role,Short_Name Dept_Name,'1st year, 1st semester' Semester from USERS,Students,Mst_Program,Mst_Dept_Office Where  " +
                  "User_Id='"+pObjectId+"' " +
  ...
  public StreamingOutput getBulk(final @Context Request pRequest, final @PathParam("object-id") String pObjectId) throws Exception {
    return new StreamingOutput() {
      public void write(OutputStream output) throws IOException, WebApplicationException {
        try {
          String query="Select User_Id,Temp_Password,Full_Name,'Student' Role,Short_Name Dept_Name,'1st year, 1st semester' Semester from USERS,Students,Mst_Program,Mst_Dept_Office Where  " +
              "Mst_Program.Program_Id='"+pObjectId+"' " +
              "And USERS.USER_ID=STUDENTS.STUDENT_ID " +
              "And MST_PROGRAM.PROGRAM_ID=STUDENTS.PROGRAM_ID " +
              "And MST_PROGRAM.DEPT_ID=MST_DEPT_OFFICE.DEPT_ID ";
          mPasswordReportGenerator.generateReport(AbstractReportGenerator.OutputType.PDF, output, query);
